        ...
        <div class="w-full p-6 shadow-lg bg-[#D9D9D9] rounded-md">
            ...
            <form action="{{ route('questions.update', $question->id) }}" method="POST">
                ...
                <div class="mt-3">
                    ...
                    @error('title') border-red-500 @enderror required" value="{{ old('title', $question->title) }}" placeholder="Ketikkan Nama Forum Diskusi..."/>
                    @error('title')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mt-3">
                    <label for="content" class="block text-base mb-2">Pertanyaan</label>
                    ...
                    @error('content') border-red-500 @enderror required" placeholder="Ketikkan Hal yang Ingin Didiskusikan...">{{ old('content', $question->content) }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-2">
                <a href="{{ route('questions.index') }}"><button type="submit" class="border-2 border-gray-400 bg-gray-400 text-white py-1 w-full rounded-md hover:bg-gray-500 hover:border-gray-500 font-semibold"><i class="bi bi-x-lg"></i> Kembali</button></a>
            </div>
        </div>
    </div>
</div>
