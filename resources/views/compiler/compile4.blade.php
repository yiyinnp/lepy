...
@section('container')
@php
    $questions = \App\Models\Question::where('id', 4)->value('content');
    $correctAnswer = \App\Models\Answer::where('id', 4)->value('content');
        ...
        <form id="compilerForm" class="mx-auto bg-[#395886] p-5 rounded-lg shadow-md">
            ...
            <div class="flex justify-between">
                <div>
                    ...
                    <a href="{{ route('material.variable') }}"><button type="button" class="ml-2 bg-yellow-300 text-gray-700 px-4 py-2 rounded hover:bg-yellow-400 focus:outline-none focus:ring focus:border-yello-500"><i class="bi bi-arrow-left"></i> Back</button></a>
                </div>
                <div>
                    <a href="{{ route('compile.variable.3') }}"><button type="button" class="ml-2 bg-blue-300 text-gray-700 px-4 py-2 rounded hover:bg-blue-400 focus:outline-none focus:ring focus:border-blue-500"><i class="bi bi-caret-left-fill"></i></button></a>
                    <a href="{{ route('compile.variable.5') }}"><button type="button" id="compileBtn5" class="ml-2 bg-blue-300 text-gray-700 px-4 py-2 rounded hover:bg-blue-400 focus:outline-none focus:ring focus:border-blue-500" disabled><i class="bi bi-caret-right-fill"></i></button></a>
                </div>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let compileBtn5 = document.querySelector('#compileBtn5');
        ...
        document.getElementById('compilerForm').addEventListener('submit', function(event) {
            ...
            .then(data => {
                ...
                if (userCode === correctAnswer) {
                    ...
                    document.getElementById('error').innerText = '';
                    compileBtn5.removeAttribute('disabled');
                    ...
                    compileBtn5.setAttribute('disabled', 'disabled');
                }
            })
            .catch(error => {
