...
@section('container')
@php
    $questions = \App\Models\Question::where('id', 1)->value('content');
    $correctAnswer = \App\Models\Answer::where('id', 1)->value('content');
    ...
    <div class="flex-none w-8/12 h-96 pr-10">
        <form id="compilerForm" class="mx-auto bg-[#395886] p-5 rounded-lg shadow-md">
            ...
            <div class="flex justify-between">
                ...
                <div>
                    <a href=""><button type="button" class="ml-2 bg-blue-300 text-gray-700 px-4 py-2 rounded hover:bg-blue-400 focus:outline-none focus:ring focus:border-blue-500" disabled><i class="bi bi-caret-left-fill"></i></button></a>
                    <a href="{{ route('compile.variable.2') }}"><button type="button" id="compileBtn2" class="ml-2 bg-blue-300 text-gray-700 px-4 py-2 rounded hover:bg-blue-400 focus:outline-none focus:ring focus:border-blue-500" disabled><i class="bi bi-caret-right-fill"></i></button></a>
                </div>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let compileBtn2 = document.querySelector('#compileBtn2');
        ...
        document.getElementById('compilerForm').addEventListener('submit', function(event) {
            ...
            .then(data => {
                ...
                if (userCode === correctAnswer) {
                    ...
                    document.getElementById('error').innerText = '';
                    compileBtn2.removeAttribute('disabled');
                    ...
                    compileBtn2.setAttribute('disabled', 'disabled');
                }
            })
            .catch(error => {
