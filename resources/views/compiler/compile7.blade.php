...
@section('container')
@php
    $questions = \App\Models\Question::where('id', 7)->value('content');
    $correctAnswer = \App\Models\Answer::where('id', 7)->value('content');
        ...
        <form id="compilerForm" class="mx-auto bg-[#395886] px-4 pb-4 lg:mb-0 mb-5 pt-2 rounded-lg shadow-md">
            ...
            <div class="flex justify-between items-center hidden sm:flex">
                <div>
                    ...
                    <a href="{{ route('material.datatype') }}"><button type="button" class="ml-2 bg-yellow-300 text-gray-700 px-3 py-2 mt-1 rounded hover:bg-yellow-400"><i class="bi bi-arrow-left"></i> Back</button></a>
                </div>
                <div>
                    <a href="{{ route('compile.tipedata.1') }}"><button type="button" class="ml-2 bg-blue-300 text-gray-700 px-3 py-2 mt-1 rounded hover:bg-blue-400"><i class="bi bi-caret-left-fill"></i></button></a>
                    <label for="" class="text-center ml-2 mt-1 font-medium text-white">2/5</label>
                    <a href="{{ route('compile.tipedata.3') }}"><button type="button" id="compileBtn7" class="ml-2 bg-blue-300 text-gray-700 px-3 py-2 mt-1 rounded hover:bg-blue-400"><i class="bi bi-caret-right-fill"></i></button></a>
                </div>
            </div>
            <div class="flex justify-between items-center md:hidden">
                <div>
                    ...
                    <a href="{{ route('material.datatype') }}"><button type="button" class="ml-2 bg-yellow-300 text-gray-700 px-3 py-2 mt-1 rounded hover:bg-yellow-400"><i class="bi bi-arrow-left"></i></button></a>
                </div>
                <div>
                    <a href="{{ route('compile.tipedata.1') }}"><button type="button" class="ml-2 bg-blue-300 text-gray-700 px-3 py-2 mt-1 rounded hover:bg-blue-400"><i class="bi bi-caret-left-fill"></i></button></a>
                    <label for="" class="text-center ml-2 mt-1 font-medium text-white">2/5</label>
                    <a href="{{ route('compile.tipedata.3') }}"><button type="button" class="ml-2 bg-blue-300 text-gray-700 px-3 py-2 mt-1 rounded hover:bg-blue-400"><i class="bi bi-caret-right-fill"></i></button></a>
                </div>
            </div>
        </form>
    </div>
        ...
        document.getElementById('compilerForm').addEventListener('submit', function(event) {
            ...
            .then(data => {
                ...
                if (userCode === correctAnswer) {
                    ...
                    document.getElementById('warning').innerText = '';
                    document.getElementById('error').innerText = '';
                    compileBtn7.removeAttribute('disabled');
